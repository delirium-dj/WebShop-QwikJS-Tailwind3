...
export const ProductCard = component$<ProductCardProps>((props) => {
  ...
  const images = flatImages.length > 0 ? flatImages : (nestedProduct?.images || []);
  ...
  const inStock = flatInStock !== undefined ? flatInStock : nestedProduct?.inStock;
  ...
  const isAvailable = inStock !== undefined ? inStock : (stock !== undefined ? stock > 0 : true);
  
  // Resolve primary image
  // If we have any images object, use the one marked primary.
  // Otherwise, if we have a legacy image string, wrap it.
  const displayImage: ProductImageType = images.length > 0
  ...
  const discountPct = discount || (originalPrice ? Math.round(((originalPrice - price) / originalPrice) * 100) : 0);
  const displayOriginalPrice = originalPrice || (discount ? price / (1 - discount / 100) : undefined);
